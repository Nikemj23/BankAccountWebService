...
//  } 
	...
	@PostMapping
	public ResponseEntity<?> addEmpDetails(@RequestBody @Valid Employee emp) {//@Valid==>to tell explicitly to the container don't just perform un marshalling but also validation
		//before use of @ valid client was getting internal server error
		//after @valid json error response was sent with entire stack trace to client
		//to avoid sending stack trace==> global Exception handling
		//@Valid: to send response error code (HTTP:400: Bad request) to the frontend
		try {
			Employee savedEmp = employeeService.addOrUpdateEmployeeDetails(emp);
...
//		}
	...
	@PutMapping
	public Employee updateEmpDetails(@RequestBody @Valid Employee e)// deserial (un marshalling)
	{
...
//	    public ResponseEntity<?> getEmpFullNamesByDept(@PathVariable String dept) {
